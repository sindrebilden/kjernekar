...
def _is_at_time(time, haiku):
    # This is a wery rough approach
    haiku_time = dateutil.parser.parse(haiku.date)
    diff = abs(haiku_time - time)
...
class Persistence:
    def __init__(self, db_location='./haikubot.db'):
        if db is None:
            if config.FILE_DB:
    ...
    def put_haiku(self, haiku, author, link=None, posted=False, date=str(datetime.now())):
        ...
        print ('HAIKU TIMESTAMP: {}'.format(date))
    ...
    def get_haiku_at_time(self, timestamp):
        date = datetime.fromtimestamp(timestamp)
        logging.debug('Getting haiku at {}.'.format(str(date)))

        haiku_list = list(filter(partial(_is_at_time, date), [row for row in self.connection.execute(
            select([haikus])
        )]))

        if len(haiku_list) is 0:
            logging.warn('Could not find haiku at timestamp will ignore the reaction.')
            return None

        if len(haiku_list) > 1:
            logging.warn('Found multiple haikus at timestamp, choosing the first. Shhh...')
        
        return haiku_list[0]

    def add_point_to_haiku(self, hid, point):
        logging.debug('Adding {} point to hauku #{}.'.format(point, hid))
        return
