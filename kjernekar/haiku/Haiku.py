...
class HaikuHandler:
    ...
    def store_haiku(self, haiku=None, author=None, link=None):
        ...
        if haiku_id:
            logging.debug(
                "Posting haiku #{} to {}".format(haiku_id, self.slack_channel_haiku)
            )
    ...
    def handle_slack_haiku_interactive(self, req):
        ...
        if req.payload["type"] == "block_actions":
            option = req.payload["state"]["values"]["section"]["action"][
                "selected_option"
            ]["value"]
        ...
        if (
            req.payload["type"] == "view_submission"
            and req.payload["view"]["callback_id"] == "new-haiku-modal"
        ):
    ...
    def _create_new_haiku(self, req):
        ...
        user = req.payload["user"]["username"]
        link = req.payload["view"]["state"]["values"]["link-block"]["link-input"][
            "value"
        ]
        haiku = req.payload["view"]["state"]["values"]["haiku-block"]["haiku-input"][
            "value"
        ]

        self.store_haiku(haiku, user, link)

    def _haiku_handle_command(
        self, option, user=None, channel=None, req=None, acknowledge_callback=None
    ):
